...
func main() {
	...
	if err != nil {
		log.Fatal("could not listen to port 8888: %v", err)
	}
...
func (s taskServer) Add(ctx context.Context, text *todo.Text) (*todo.Task,error) {
	...
	if err != nil {
		return nil,fmt.Errorf("c ould not encode task: %v", err)
	}

	f, err :=os.OpenFile(dbPath, os.O_WRONLY|os.O_CREATE|os.O_APPEND, 0666)
	if err != nil {
		return nil, fmt.Errorf("cound not open %s: %v", dbPath, err)
	}

	if err :=binary.Write(f, endianness, length(len(b))); err != nil {
		return nil, fmt.Errorf("could not encode length of message %s", err)
	}
...
func (s taskServer) List(ctx context.Context, void *todo.Void) (*todo.TaskList, error){
	...
	for {
		...
		if err := binary.Read(bytes.NewReader(b[:sizeOfLength]), endianness, &l); err != nil {
			return nil,fmt.Errorf("cound not decode message length: %v", err)
		}
		b = b[sizeOfLength:]
